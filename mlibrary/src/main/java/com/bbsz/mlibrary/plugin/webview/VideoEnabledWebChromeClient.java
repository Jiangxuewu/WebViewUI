...
public class VideoEnabledWebChromeClient extends WebChromeClient implements MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener {
    ...
    public void init(View activityNonVideoView, ViewGroup activityVideoView, View loadingView, VideoEnabledWebView webView){
        this.activityNonVideoView = activityNonVideoView;
        this.activityVideoView = activityVideoView;
        this.loadingView = loadingView;
        this.webView = webView;
        ...
        if (view instanceof FrameLayout) {
            ...
            if (focusedChild instanceof android.widget.VideoView) {
                ...
                if (webView != null && webView.getSettings().getJavaScriptEnabled() && focusedChild instanceof SurfaceView) {
                    ...
                    js += "var _html5_video_last;";
                    js += "var _html5_video = document.getElementsByTagName('video')[0];";
                    js += "if (_html5_video != undefined && _html5_video != _html5_video_last) {";
                    {
                        js += "_html5_video_last = _html5_video;";
                        js += "function _html5_video_ended() {";
                        {
                            js +=  MWebView.control + ".notifyVideoEnd();"; // Must match Javascript interface name and method of VideoEnableWebView
                        }
                        js += "}";
                        js += "_html5_video.addEventListener('ended', _html5_video_ended);";
                    }
                    js += "}";
                    webView.loadUrl(js);
                }
